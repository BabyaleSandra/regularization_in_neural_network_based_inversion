...
def prior_cov_2d(xn, zn, sigma_m, Delta):
    n   = len(xn) * len(zn)
    idx = np.arange(n)
    px  = xn[idx % len(xn)]    # x-coordinate of node k  (Fortran order)
    pz  = zn[idx // len(xn)]   # z-coordinate of node k

    PX1, PX2 = np.meshgrid(px, px)
    PZ1, PZ2 = np.meshgrid(pz, pz)

    cov = (sigma_m**2) * np.exp(-0.5 * ((PX1-PX2)**2 + (PZ1-PZ2)**2) / Delta**2)
    return cov

sigma_m = (0.05 * (1.0 / vbackground))
Delta   = 400.0
Cm      = prior_cov_2d(xn, zn, sigma_m, Delta)
