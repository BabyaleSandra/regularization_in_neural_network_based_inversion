...
# ── Problem geometry (In[3,4]) ────────────────────────────────────────
H          = 10.0
m          = 100                                # In[3]: m=100
wmin, wmax = 0.0, 100.0
w          = np.linspace(wmin, wmax, m)         # parameter grid (100,)
n          = 15                                 # In[4]: n=15
x          = np.linspace(wmin, wmax, n)         # observation grid (15,)
sigma_d    = 0.1
sigma_z    = 1                                  # In[3]: sigma_z=1
Delta      = 5
maxz       = 2.5

# ── Physics tensors (In[13]) ──────────────────────────────────────────
x_tf  = tf.constant(x,           dtype=tf.float32)   # (15,)
w_tf  = tf.constant(w,           dtype=tf.float32)   # (100,)
dw_tf = tf.constant(w[1] - w[0], dtype=tf.float32)
...
# ══════════════════════════════════════════════════════════════════════
# Forward operator  (In[13] — fix1: proper trapz; fix2: 1e-30 floor)
#
# u(x_i) = integral_a^b log[ ((x-w)^2 + H^2) /
#                             ((x-w)^2 + (H-z(w))^2) ] dw
#
# Input  z : (B, 100)  physical (unscaled) units
# Output u : (B, 15)
# ══════════════════════════════════════════════════════════════════════
@tf.function
def forward_G_tf(z):
    x_e = tf.reshape(x_tf, (1, -1, 1))                         # (1,15,1)
    w_e = tf.reshape(w_tf, (1,  1, -1))                        # (1,1,100)
    z_e = tf.reshape(z, (tf.shape(z)[0], 1, tf.shape(z)[1]))   # (B,1,100)

    num       = (x_e - w_e)**2 + H_tf**2                       # (1,15,100)
    den       = (x_e - w_e)**2 + (H_tf - z_e)**2 + 1e-30      # (B,15,100)
    integrand = tf.math.log(num / den)                          # (B,15,100)

    # Proper trapezoidal rule (fix1)
    u = dw_tf * (
        tf.reduce_sum(integrand, axis=-1)
...
# ══════════════════════════════════════════════════════════════════════
# Physics-informed loss  (In[13])
#
# Term 1: supervised MSE in scaled space
# Term 2: physics residual — G(z_pred_phys) vs d_orig_batch
#
# d_orig_batch is the UNSCALED observation paired with each training
# sample (unscaled X_batch), NOT the single fixed test datum d_obs.
# ══════════════════════════════════════════════════════════════════════
def make_physics_loss(z_mean_tf, z_std_tf):
    ...
    def physics_informed_loss(y_batch_scaled, z_pred_scaled, d_orig_batch):
        ...
        z_pred_phys = z_pred_scaled * z_std_tf + z_mean_tf     # (B,100)
        z_pred_phys = tf.maximum(z_pred_phys, 1e-4)            # fix3: safety

        # Physics residual against each sample's own observation
        u_pred = forward_G_tf(z_pred_phys)                     # (B,15)
...
# ══════════════════════════════════════════════════════════════════════
# Train / val step factories  (In[13])
# ══════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════
# Custom training loop  (In[14])
#
# Faithful to PINN_softplus.ipynb:
#   - d_orig shuffled in lockstep with X and y (fix5)
...
def train_physics_informed(model, X_train, y_train, X_val, y_val,
    ...
    for epoch in range(epochs):
        # Shuffle X, y, d_orig in lockstep (fix5)
        ...
        for s in range(steps):
            Xb = X_sh[s * batch_size : (s + 1) * batch_size]
            yb = y_sh[s * batch_size : (s + 1) * batch_size]
            db = d_sh[s * batch_size : (s + 1) * batch_size]
...
# ══════════════════════════════════════════════════════════════════════
# Model builders  (In[7], In[8], In[9-12])
# Softplus fused into final Dense: Dense(output_dim, activation="softplus")
# ══════════════════════════════════════════════════════════════════════

def build_mlp_model(input_dim, output_dim):
    """In[7]: [128->256->512->256->128] + residual, softplus output."""
    ...
    # In[7]: activation="softplus" fused into Dense
    outputs = layers.Dense(output_dim, activation="softplus", name="z")(x)
    return keras.Model(inputs, outputs, name="MLP")


def build_cnn_model(input_dim, output_dim):
    """In[8]: parallel Conv1D branches, dense head, softplus output."""
    ...
    feat = [branch(k) for k in kernels]
    h    = layers.Concatenate()(feat)
    ...
    # In[8]: activation="softplus" fused into Dense
    out = layers.Dense(output_dim, activation="softplus", name="z")(h)
    return keras.Model(inp, out, name="CNN")


def build_deeponet_mlp_fourier(branch_dim, n_z):
    """
    In[9-12]: MLP branch + Fourier trunk + softplus on einsum output.
    Trunk fix: proper keras.Model with Input so weights are trained.
    """
    # Fourier features of fixed w-grid (In[9]: fourier_features)
    ...
    # Trunk sub-model (In[9]: trunk_matrix, fourier variant)
    phi_dim   = 1 + 2 * n_freqs
    ...
    # Branch sub-model (In[10]: branch_mlp)
    ...
    # Trunk eval inside graph (trunk fix)
    T = layers.Lambda(
    ...
    # In[11]: softplus after einsum
    out = layers.Activation("softplus", name="softplus_out")(field)
...
for prior_name, (z_file, d_file) in PRIORS.items():
    ...
    # Scaler parameters for inverse transform inside TF loss
    scaler_z_mean_np  = scaler_z.mean_.astype(np.float32)
    scaler_z_scale_np = scaler_z.scale_.astype(np.float32)

    # Test observation scaled for prediction
    d_scaled = scaler_G.transform(d_obs.reshape(1, -1))

    # ── Train / val split (In[5]) ─────────────────────────────────────
    X_train, X_val, y_train, y_val = train_test_split(
        Gz_scaled, Z_scaled, test_size=0.15, random_state=42
    )

    # ── d_orig: unscale X back to physical observations (In[13]) ─────
    ...
    # ── Build models — fresh weights each prior (In[12]) ─────────────
    all_models = {
        "MLP": build_mlp_model(input_dim, output_dim),
        "CNN": build_cnn_model(input_dim, output_dim),
        "DeepONet_MLP_Fourier": build_deeponet_mlp_fourier(
...
# ══════════════════════════════════════════════════════════════════════
# Save CSVs — same schema as NN_1D_all_priors.py for joint analysis
# ══════════════════════════════════════════════════════════════════════

# ── gravity_pinn_predictions.csv ──────────────────────────────────────
rows_predictions = []
...
pd.DataFrame(rows_predictions).to_csv(
    os.path.join(RESULTS_DIR, "gravity_pinn_predictions.csv"), index=False
)

# ── gravity_pinn_metrics.csv ──────────────────────────────────────────
rows_metrics = []
...
pd.DataFrame(rows_metrics).to_csv(
    os.path.join(RESULTS_DIR, "gravity_pinn_metrics.csv"), index=False
)

# ── gravity_pinn_histories.csv ────────────────────────────────────────
rows_hist = []
...
pd.DataFrame(rows_hist).to_csv(
    os.path.join(RESULTS_DIR, "gravity_pinn_histories.csv"), index=False
)

print("\nSaved files:")
print(os.path.join(RESULTS_DIR, "gravity_pinn_predictions.csv"))
print(os.path.join(RESULTS_DIR, "gravity_pinn_metrics.csv"))
print(os.path.join(RESULTS_DIR, "gravity_pinn_histories.csv"))
